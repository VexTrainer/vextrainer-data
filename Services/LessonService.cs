...
public class LessonService
{
  ...
  /// list, up-next lesson, recent activity, per-module progress, last
  /// quiz attempt, and the user's bookmarks.
  ///
  ...
  public async Task<ApiResponse<UserWebDashboard>> GetUserWebDashboardAsync(int userId) {
    ...
    // Read the seven result sets in the order the stored procedure emits them
    ...
    var bookmarks = (await multi.ReadAsync<Bookmark>()).ToList();
  ...
  /// in a single round trip: stats (with reading streak), the
  /// continue-learning list, and the user's bookmarks. Same stored
  /// procedure shape as the web dashboard, trimmed to the three result
  /// sets the app needs.
  ///
